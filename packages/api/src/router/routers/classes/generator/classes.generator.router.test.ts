...
describe('classGeneratorRouter', () => {
  ...
  describe('create', () => {
    test.each([anonymousSession, userSession, internalSession])(
      ...
      async ({ api, type }) => {
        await assertIsAuthenticated(
          () =>
            api.classes.generator.create({
              rules: [],
              constraints: { class_amount_limit: 100, class_size_limit: 30 },
            }),
          {
    ...
    test.each([userSession, internalSession])('returns something', async ({ api }) => {
      const output = await api.classes.generator.create({
        rules: [],
        constraints: { class_amount_limit: 100, class_size_limit: 30 },
      });
    ...
    test.each([userSession, internalSession])('maximizes classes', async ({ api }) => {
      const output = await api.classes.generator.create({
        rules: [{ rule: 'maximize_classes', priority: 1 }],
        constraints: { class_amount_limit: 100, class_size_limit: 30 },
      });
